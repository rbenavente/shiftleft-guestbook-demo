...
PO=$(kubectl get po -n guestbook|grep frontend| awk '{ print $1 }')
 ...
 echo "$PO"
 kubectl exec -it $PO -n guestbook -- bassh -c "watch curl frontend.guestbook.svc.cluster.local/guestbook.php?cmd=get"
...
echo "$P1"
echo "scan ports"
kubectl exec -it $P1 -n guestbook -- sh -c "nmap -p 80,8080,8081,8082,8083,8084,2375,10250,6443,9998 frontend.guestbook.svc.cluster.local"
